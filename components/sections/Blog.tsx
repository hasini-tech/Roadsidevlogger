  ...
  return (
    <section id="blog" className="bg-white py-24 dark:bg-asphalt-900 sm:py-32">
      <div className="container-luxe">
        ...
        {/* Mobile/tablet: horizontal snap carousel | lg+: 3-column grid */}
        <div
          className="
        ...
        >
          {blogPosts.map((post, i) => (
            <motion.a
              ...
              className="
                group flex w-[82%] flex-shrink-0 snap-center flex-col overflow-hidden
                rounded-2xl shadow-luxury transition-transform hover:-translate-y-1
                sm:w-[46%] lg:w-auto lg:flex-shrink
              "
            >
        ...
        {/* Scroll hint dots, hidden at lg+ */}
        <div className="mt-4 flex justify-center gap-1.5 lg:hidden">
          {blogPosts.map((post) => (
            <span
              key={post.id}
              className="h-1.5 w-1.5 rounded-full bg-steel-500/30 dark:bg-cream-100/30"
            />
          ))}
        </div>
      </div>
