  ...
  return (
    <section id="videos" className="bg-cream-100 py-24 dark:bg-asphalt-950 sm:py-32">
      <div className="container-luxe">
        ...
        <div
          className="
            -mx-4 flex snap-x snap-mandatory gap-5 overflow-x-auto px-4 pb-4
            [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden
            sm:mx-0 sm:grid sm:snap-none sm:gap-8 sm:overflow-visible sm:px-0 sm:pb-0
        ...
        >
          {featuredVideos.map((service, i) => (
            <motion.article
              ...
              className="group w-[80%] shrink-0 snap-center overflow-hidden rounded-2xl bg-white shadow-luxury transition-transform duration-300 hover:-translate-y-1 dark:bg-asphalt-800 sm:w-auto sm:shrink"
            >
        ...
        <div className="mt-4 flex justify-center gap-1.5 sm:hidden">
          {featuredVideos.map((service) => (
            <span
              key={service.id}
              className="h-1.5 w-1.5 rounded-full bg-steel-500/30"
            />
          ))}
        </div>
      </div>
